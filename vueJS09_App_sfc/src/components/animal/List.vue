<template>
    <div class="container is-fluid mt-3">
         <router-link to="/animalForm">
            <button class="button is-info ">
                <i class='fas fa-plus-circle mr-1'></i> 
                Nuevo registro
            </button>
         </router-link> <br><br>
        <h4 class="is-size-3 has-text-centered ">Listado de animales</h4>
            ...
            <tbody>
                <tr v-for="(item, index) in animales" :key="index">
                    ...
                    <td>
                        ...
                        <router-link to="/animalForm">
                            <button class="button is-small is-warning mb-1" @click="cargarAnimal(item,index)">
...
export default {
    ...
    computed: {
        animales(){
            return this.$store.getters.getAnimales
        }
    },
    methods: {
        eliminar(key){
            this.$store.dispatch('deleteAnimal',key)
        },
        cargarAnimal(valor,key){
            valor.key=key
            this.$store.dispatch('loadAnimal',{valor:valor,estado:'modificar'})
        }
    },
}
</script>
